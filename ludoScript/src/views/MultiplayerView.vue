    ...
    <main class="flex-grow flex flex-col items-center justify-center px-4 py-8">
      ...
      <transition name="fade">
        ...
        >
          {{ mp.error }}
          <button
            class="ml-3 underline opacity-70 hover:opacity-100"
            @click="mp.error = null"
          >
            OK
          </button>
      ...
      <template v-if="mp.status === 'idle'">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 items-center gap-6">
            <div class="flex justify-center">
              <img
                :src="IMAGES.multi"
                alt="multijugador"
                class="max-w-[240px] w-full md:max-w-full lg:ml-96"
              >
            </div>
            <div class="flex flex-col items-center gap-8 max-w-md w-full mx-auto">
              <div class="text-center ">
                <h1 class="text-white text-4xl font-black mb-2 font-righteous">
                  Multijugador
                </h1>
                <p class="text-white/60">
                  Juega contra tus amigos en tiempo real
                </p>
              </div>
        ...
        <RoomCreate
          v-else-if="subView === 'create'"
          @cancel="subView = null"
        />
        <RoomJoin
          v-else-if="subView === 'join'"
          @cancel="subView = null"
        />
      </template>
