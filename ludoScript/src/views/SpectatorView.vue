<template>
    <div class="min-h-screen flex flex-col bg-gray-900">
        ...
        <main class="flex-grow flex flex-col items-center justify-center px-4 py-8">
            ...
            <div v-if="store.loading" class="flex flex-col items-center gap-4 text-center">
                <i class="pi pi-spin pi-spinner text-4xl text-indigo-400"></i>
                <p class="text-slate-300 font-medium">Conectando a la partida…</p>
                <p class="text-slate-500 text-sm">Sala: {{ routeCode }}</p>
            </div>

            <!-- H9: error con mensaje humano y opción de reintentar -->
            <div v-else-if="store.error" class="max-w-sm w-full text-center">
                <div class="bg-red-900/30 border border-red-500/30 rounded-2xl p-6">
                    <i class="pi pi-exclamation-circle text-red-400 text-3xl mb-3 block"></i>
                    <p class="text-red-200 font-semibold mb-1">No se pudo ver la partida</p>
                    <p class="text-red-400/70 text-sm mb-4">{{ store.error }}</p>
                    <button @click="retry"
            ...
            <!-- Sala cerrada -->
            <div v-else-if="store.gameStatus === 'closed'" class="text-center">
                <p class="text-slate-300 text-xl font-bold mb-2">La sala ha sido cerrada</p>
                <p class="text-slate-500 text-sm">El profesor ha terminado la sesión.</p>
            </div>

            <!-- ── LOBBY: esperando inicio ── -->
            <div v-else-if="store.gameStatus === 'lobby' || store.gameStatus === 'idle'"
                class="w-full max-w-2xl">
                <div class="text-center mb-8">
                    <i class="pi pi-hourglass text-4xl text-indigo-400 mb-3 block"></i>
                    <h1 class="text-white text-2xl font-bold mb-1">Esperando que la partida comience</h1>
                    <p class="text-slate-400 text-sm">El profesor iniciará la partida en breve.</p>
                </div>
                <!-- Lista de jugadores en lobby -->
                <div v-if="store.scores.length" class="bg-gray-800/60 rounded-2xl p-4 border border-white/5">
                    <p class="text-slate-400 text-xs font-medium uppercase tracking-wider mb-3">
                        Jugadores ({{ store.scores.length }})
                    </p>
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
                        <div v-for="p in store.scores" :key="p.userId"
                            ...
                            <span v-if="p.isHost" class="ml-auto text-xs text-yellow-400">👑</span>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div v-else-if="store.gameStatus === 'playing'" class="w-full max-w-5xl">
                <div class="flex flex-col lg:flex-row gap-6">

                    <!-- Columna izquierda: pregunta -->
                    <div class="flex-grow">
                        <div v-if="store.currentQuestion" class="flex flex-col gap-4">
                            ...
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    <span :class="store.timerRemaining <= 5 ? 'text-red-400' : 'text-slate-300'"
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                <div v-for="(opt, idx) in store.currentQuestion.options" :key="idx"
                                    ...
                                    <span v-if="store.revealedCorrectIndex === idx" class="ml-2">✓</span>
                                </div>
                            </div>

                            <!-- Explicación tras reveal -->
                            <transition name="fade">
                                <div v-if="store.lastExplanation && store.revealedCorrectIndex !== null"
                        ...
                        <div v-else class="space-y-4">
                            <div class="h-8 bg-gray-700/50 rounded-xl animate-pulse"></div>
                            <div class="h-24 bg-gray-700/50 rounded-2xl animate-pulse"></div>
                            <div class="grid grid-cols-2 gap-3">
                                <div v-for="i in 4" :key="i" class="h-12 bg-gray-700/50 rounded-xl animate-pulse"></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="lg:w-64 shrink-0">
                        <div class="bg-gray-800/60 border border-white/5 rounded-2xl p-4 sticky top-4">
                            ...
                            <div class="space-y-2">
                                <div v-for="(p, i) in sortedScores" :key="p.userId"
            ...
            <div v-else-if="store.gameStatus === 'finished'" class="w-full max-w-lg text-center">
                <h1 class="text-white text-3xl font-black mb-2">¡Partida terminada!</h1>
                <p class="text-slate-400 text-sm mb-8">Resultados finales</p>

                <!-- Podio top 3 -->
                <div class="flex items-end justify-center gap-4 mb-8">
                    <div v-for="(p, i) in podium" :key="p?.userId ?? i"
                ...
                <div class="bg-gray-800/60 border border-white/5 rounded-2xl overflow-hidden">
                    <div v-for="(p, i) in store.finalRanking" :key="p.userId"
