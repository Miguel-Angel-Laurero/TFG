<template>
    <div class="space-y-5">
        ...
        <div class="bg-gray-900/60 rounded-2xl overflow-hidden border border-white/10">
            ...
            <div class="divide-y divide-white/5">
                <div v-for="(entry, i) in entries" :key="entry.id"
                    ...
                    <input v-model="entry.key" placeholder="clave" maxlength="20"
                    ...
                    <input v-model="entry.value" placeholder="valor" maxlength="30"
                ...
                <div v-if="entries.length === 0" class="px-4 py-4 text-center text-gray-600 text-xs font-mono">
                    objeto vacío — { }
                </div>
            </div>
        ...
        <div class="flex flex-wrap gap-2 items-center text-xs font-mono">
            ...
            <div v-if="entries.length > 0" class="flex gap-1 items-center flex-wrap">
                ...
                <select v-model="selectedKey"
                    ...
                    <option v-for="e in entries.filter(e => e.key)" :key="e.id" :value="e.key">{{ e.key }}</option>
                </select>
                <span class="text-gray-500">→</span>
                <span class="text-green-400 font-bold">{{ selectedValue }}</span>
            </div>
            <span v-else class="text-gray-600">— añade propiedades para ver</span>
        </div>
        ...
        <div class="rounded-xl overflow-hidden border border-white/10">
            ...
            <pre
                class="bg-gray-950/60 px-4 py-3 text-sm font-mono overflow-x-auto text-gray-300 leading-relaxed">{{ jsonPreview }}</pre>
        </div>
    </div>
</template>
