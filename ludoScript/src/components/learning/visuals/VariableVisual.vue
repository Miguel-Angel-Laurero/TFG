<template>
  <div class="space-y-5">
    ...
    <div class="flex justify-center py-4">
      <div class="flex flex-col items-center">
        ...
        >
          ...
          <!-- Lock icon for const -->
          <i
            v-if="keyword === 'const' && locked"
            class="pi pi-lock absolute top-1.5 right-2 text-yellow-500/60 text-xs"
          />
        </div>
        <!-- Address label -->
        <p class="text-xs text-gray-600 font-mono mt-1.5">
          0x{{ memAddr }}
        </p>
      </div>
    </div>
    ...
    <div class="rounded-xl overflow-hidden border border-white/10">
      ...
      <pre
        class="bg-gray-950/60 px-4 py-3 text-sm font-mono overflow-x-auto"
      ><code><span :class="keyword === 'const' ? 'text-yellow-400' : 'text-blue-400'">{{ keyword }}</span><span class="text-white"> {{ displayName }}</span><span class="text-gray-400"> = </span><span :class="isNumber ? 'text-orange-300' : 'text-green-300'">{{ codeValue }}</span><span class="text-gray-500">;</span>
<span
v-if="keyword === 'const' && locked"
      class="text-gray-600"
      >// {{ displayName }} = "otro valor"; ← Error: Assignment to constant</span></code></pre>
    </div>
