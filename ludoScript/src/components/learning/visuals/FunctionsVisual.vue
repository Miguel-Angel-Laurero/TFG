<template>
    <div class="space-y-5">
        ...
        <div class="flex flex-wrap gap-2">
            <button v-for="fn in FUNCTIONS" :key="fn.id" @click="selectFn(fn)" :class="[
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 bg-gray-900/60 rounded-2xl p-4">
            ...
            <div class="flex flex-col gap-2 items-center sm:items-end">
                <div v-for="(param, i) in selected.params" :key="param.name" class="flex items-center gap-2">
                    <label class="text-xs text-gray-400 font-mono w-4 text-right">{{ param.name }}:</label>
                    <input v-model="paramValues[i]" :type="param.type === 'number' ? 'number' : 'text'"
                        :placeholder="param.placeholder" @input="clearResult"
                ...
                <div class="text-gray-600 text-xs text-right font-mono">parámetros →</div>
            </div>
            ...
            <div class="flex-1 bg-gray-800 border-2 border-orange-500/40 rounded-2xl p-4 text-center min-w-[130px]">
                <p class="text-xs text-gray-500 mb-1 font-mono">función</p>
                <p class="text-orange-400 font-bold font-mono text-sm">⚙️ {{ selected.name }}</p>
                <p class="text-xs text-gray-600 mt-1 font-mono">{{ selected.internals }}</p>
            </div>
            ...
            <div class="flex flex-col items-center sm:items-start gap-2">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Transition name="pop">
                        <span v-if="result !== null" :key="result"
                            class="font-mono font-bold text-green-400 text-base bg-green-500/10 border border-green-500/30 px-3 py-0.5 rounded-lg">
                            {{ result }}
                        </span>
                        <span v-else class="text-gray-600 font-mono text-sm">?</span>
                    </Transition>
                </div>
                <p v-if="errorMsg" class="text-xs text-red-400 font-mono">{{ errorMsg }}</p>
                <div class="text-gray-600 text-xs font-mono">← resultado</div>
            </div>
        </div>
...
<span v-if="result !== null" class="text-gray-600">// → {{ result }}</span></code></pre>
        </div>
    </div>
</template>
