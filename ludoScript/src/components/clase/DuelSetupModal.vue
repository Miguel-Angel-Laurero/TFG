    ...
    <div class="bg-gray-800 border border-white/10 rounded-2xl p-6 w-full max-w-md shadow-2xl">
      ...
      <div class="flex justify-between items-center mb-5">
        <h2 class="text-white font-bold text-xl flex items-center gap-2">
          <i class="pi pi-bolt text-yellow-400" />
          Duelo 1v1
        </h2>
      ...
      >
        <i class="pi pi-exclamation-triangle text-red-400 text-sm mt-0.5 shrink-0" />
        <div class="flex-1">
          <p class="text-red-300 text-sm">
            {{ duel.error }}
          </p>
          <p
            v-if="offlineNames.length"
            class="text-red-400/70 text-xs mt-0.5"
          >
            Sin conexión: {{ offlineNames.join(', ') }}
          </p>
        </div>
        <button
          class="text-slate-400 hover:text-white text-base leading-none"
          @click="duel.clearError()"
        >
          &times;
        </button>
      </div>
      ...
      <div
        v-if="duel.duelStatus === 'waiting'"
        class="text-center py-8"
      >
        <i class="pi pi-spin pi-spinner text-4xl text-indigo-400 mb-4 block" />
        <p class="text-white font-semibold">
          Invitaciones enviadas
        </p>
        <p class="text-slate-400 text-sm mt-1">
          Esperando que ambos jugadores acepten…
        </p>
        ...
        <p class="text-slate-500 text-xs mt-3">
          La sala caduca en 60 s si no se acepta.
        </p>
      </div>
      ...
      <div
        ...
        class="text-center py-8"
      >
        <i class="pi pi-times-circle text-4xl text-red-400 mb-4 block" />
        <p class="text-white font-semibold">
          Duelo cancelado
        </p>
        <p class="text-slate-400 text-sm mt-1">
        ...
        <div class="flex flex-col gap-4">
          ...
          <div>
            <label class="block text-slate-300 text-sm font-medium mb-1">
              <i class="pi pi-user mr-1 text-indigo-400" /> Jugador 1
            </label>
          ...
          <div>
            <label class="block text-slate-300 text-sm font-medium mb-1">
              <i class="pi pi-user mr-1 text-pink-400" /> Jugador 2
            </label>
            <select
          ...
          <div>
            <label class="block text-slate-300 text-sm font-medium mb-1">
              <i class="pi pi-tag mr-1 text-yellow-400" /> Categoría
            </label>
            <!-- H1: skeleton mientras carga -->
            <div
              v-if="loadingCategories"
              class="h-10 bg-gray-700 rounded-xl animate-pulse"
            />
            <select
              v-else
          ...
          <div>
            <label class="block text-slate-300 text-sm font-medium mb-2">
              <i class="pi pi-list mr-1 text-slate-400" />
