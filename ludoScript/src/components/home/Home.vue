<template>
  <main
    class="relative h-full w-full overflow-hidden flex flex-col md:flex-row"
    @mousemove="onDrag"
    @mouseup="stopDrag"
    @mouseleave="stopDrag"
  >
    ...
    >
      ...
      <div class="flex items-center justify-between pr-12 md:pr-0 md:mb-4 mb-2">
        <h3 class="text-indigo-300 text-sm font-bold uppercase tracking-wider">
          Configuración
        </h3>
      ...
      <HomePdfPanel
        ...
        v-model:pdf-count="pdfCount"
        v-model:selected-predefined="selectedPredefined"
      />
    </aside>
    <div
    ...
    >
      <div class="w-full max-w-[430px] md:max-w-2xl px-3.5 md:px-8">
        <h2 class="hidden md:block text-2xl md:text-3xl font-righteous text-white mb-6 text-center md:text-left">
          Modos
          de estudio
        </h2>
        ...
        <GameGrid
          ...
          :pdf-count="pdfCount"
          :selected-predefined="selectedPredefined"
