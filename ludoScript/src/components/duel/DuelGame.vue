<template>
    <div class="flex flex-col items-center gap-6 w-full max-w-3xl mx-auto p-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
            <button v-for="(option, idx) in duel.currentQuestion?.options" :key="idx"
                :disabled="duel.hasAnswered || duel.timerRemaining === 0" @click="handleAnswer(idx)"
        ...
        <transition name="fade">
            <div v-if="duel.lastAnswerResult" class="w-full rounded-2xl px-5 py-4 text-center"
                ...
                <p v-if="duel.lastAnswerResult.correct" class="text-white/70 text-sm mt-1">
                    +{{ duel.lastAnswerResult.pointsEarned }} puntos
                </p>
                <p class="text-white/50 text-xs mt-1">Total: {{ duel.lastAnswerResult.totalScore }} pts</p>
            </div>
        </transition>

        <!-- Respuesta correcta cuando se agota el tiempo sin responder -->
        <transition name="fade">
            <div v-if="duel.lastQuestionEnd && !duel.lastAnswerResult"
        ...
        <div class="w-full bg-white/5 rounded-2xl p-4">
            <h4 class="text-white/60 text-xs uppercase tracking-wide mb-2">Marcador</h4>
