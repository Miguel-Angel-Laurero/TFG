    ...
    <div class="flex flex-col items-center justify-center gap-6 w-full max-w-sm mx-auto py-20 px-4 text-center">
        <div class="relative">
            <div class="w-20 h-20 rounded-full bg-indigo-900/40 border border-indigo-500/30 flex items-center justify-center mx-auto">
                <i class="pi pi-bolt text-yellow-400 text-4xl"></i>
            </div>
        ...
        <div>
            <h2 class="text-white font-black text-2xl">Duelo 1v1</h2>
            <p class="text-slate-400 text-sm mt-1">{{ statusMessage }}</p>
        </div>

        <div v-if="duel.duelCode" class="bg-indigo-900/30 border border-indigo-500/20 rounded-2xl px-6 py-3">
            <p class="text-slate-400 text-xs uppercase tracking-widest mb-1">Código de duelo</p>
            <p class="text-indigo-300 font-mono font-bold text-xl tracking-widest">{{ duel.duelCode }}</p>
        </div>

        <!-- Jugadores que han aceptado -->
        <div v-if="duel.players.length" class="w-full bg-white/5 rounded-2xl p-4 space-y-2">
            <p class="text-slate-400 text-xs uppercase tracking-wide mb-2">Jugadores listos</p>
            <div v-for="p in duel.players" :key="p.userId"
