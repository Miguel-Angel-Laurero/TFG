<template>
  <div class="flex flex-col items-center gap-6 w-full max-w-3xl mx-auto p-4">
    ...
    <div class="w-full flex justify-between items-center text-white/60 text-sm font-medium">
      <button
        class="flex items-center gap-1.5 text-red-400/70 hover:text-red-400 hover:bg-red-500/10 px-3 py-1.5 rounded-lg transition-colors cursor-pointer group"
        @click="mp.leaveRoom()"
      >
        <i class="pi pi-sign-out text-xs group-hover:-translate-x-0.5 transition-transform" />
        Abandonar
      </button>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 w-full">
      <button
        v-for="(option, idx) in mp.currentQuestion?.options"
        :key="idx"
        :disabled="mp.hasAnswered || mp.timerRemaining === 0"
    ...
    <transition name="fade">
      ...
      >
        <p
          ...
          :class="mp.lastAnswerResult.correct ? 'text-emerald-300' : 'text-red-300'"
        >
          {{ mp.lastAnswerResult.correct ? 'Correcto' : 'Incorrecto' }}
        </p>
        <p
          v-if="mp.lastAnswerResult.correct"
          class="text-white/70 text-sm mt-1"
        >
          +{{ mp.lastAnswerResult.pointsEarned }} puntos
        </p>
        <p class="text-white/50 text-xs mt-1">
          Total: {{ mp.lastAnswerResult.totalScore }} pts
        </p>
      </div>
    </transition>
    ...
    <transition name="fade">
      <div
        v-if="mp.lastQuestionEnd && !mp.lastAnswerResult"
