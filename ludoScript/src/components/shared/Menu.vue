    ...
    <div class="flex items-center gap-2 sm:gap-4">
        ...
        <template v-else-if="auth.ready && auth.isLoggedIn">
            ...
            <RouterLink v-if="!isInShop" to="/shop-view/"
                ...
                <span
                    class="bg-yellow-400 text-black font-extrabold text-[10px] sm:text-sm px-2 py-0.5 sm:px-3 sm:py-1 rounded-full flex items-center gap-1 sm:gap-2">
                    <span>{{ auth.userData?.coins ?? 0 }}</span>
                    <img :src="IMAGES.coin" alt="coin" class="w-4 h-4 sm:w-6 sm:h-6 object-contain">
                </span>
            </RouterLink>

            <button type="button" class="cursor-pointer hover:opacity-80 transition-opacity flex-shrink-0"
                @click="toggle">
