<template>
    ...
    <button 
      ...
      class="hover:bg-white border border-gray-200 rounded-xl p-4 hover:cursor-pointer font-bold transition-all"
    >
      Volver al principio
    </button>
  </div>
  <div v-else class="flex flex-col items-center gap-8 p-10">
    ...
    <div v-if="currentCard" class="group w-100 h-64 cursor-pointer" @click="flip">
      <div 
    ...
    <!-- Siguiente: visible mientras no se haya terminado -->
    <button 
      v-if="progress < 100"
      @click.stop="handleNext" 
      class="bg-white border border-gray-200 rounded-xl p-4 hover:cursor-pointer font-bold transition-all"
    >
      Siguiente Carta →
    </button>
  </div>
