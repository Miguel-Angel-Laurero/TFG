      ...
      <form @submit.prevent="handleRegister" class="flex flex-col gap-5">
        ...
        <div class="flex flex-col gap-1.5">
          ...
          <InputText id="email" v-model="form.email" type="email" placeholder="ejemplo@correo.com"
            ...
            :class="{ 'p-invalid': auth.error }" required />
        </div>

        <div class="flex flex-col gap-1.5">
          <label for="username" class="text-xs md:text-sm font-medium text-gray-200 ml-1">Nombre de usuario</label>
          <InputText id="username" v-model="form.username" type="text" placeholder="Tu apodo"
            class="w-full p-3 !bg-white/5 !border-white/20 !text-white focus:!border-yellow-400" required />
        </div>

        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div class="flex flex-col gap-1.5">
            <label for="pass" class="text-xs md:text-sm font-medium text-gray-200 ml-1">Contraseña</label>
            <Password id="pass" v-model="form.password" toggleMask placeholder="••••••••" class="w-full" :pt="{
          ...
          <div class="flex flex-col gap-1.5">
            ...
            <Password id="confirmPass" v-model="form.confirmPassword" :feedback="false" toggleMask
              placeholder="••••••••" class="w-full" :pt="{
                input: { class: 'w-full p-3 !bg-white/5 !border-white/20 !text-white focus:!border-yellow-400' }
              }" :class="{ 'p-invalid': passwordMismatch }" required />
          </div>
        </div>

        <small v-if="usernameTooShort" class="text-red-400 ml-1 -mt-2 animate-pulse text-xs">
          El nombre de usuario debe tener al menos 3 caracteres
        </small>
        <small v-if="passwordTooShort" class="text-red-400 ml-1 -mt-2 animate-pulse text-xs">
          La contraseña debe tener al menos 8 caracteres
        </small>
        <small v-if="passwordMismatch" class="text-red-400 ml-1 -mt-2 animate-pulse text-xs">
        ...
        <Button type="submit" label="Registrarse" icon="pi pi-user-plus" :loading="auth.loading"
        ...
        <p class="text-center text-gray-400 text-sm mt-2">
          ¿Ya tienes cuenta?
          <router-link to="/login-view/" class="text-yellow-400 font-bold hover:underline ml-1">Inicia
            sesión</router-link>
        </p>
      </form>
    </div>
