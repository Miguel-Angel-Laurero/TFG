...
export function useFortnightData() {
    ...
    const performanceStatus = computed(() => {
        ...
        if (accuracy >= 90) return {
            text: '¡Rendimiento Maestro!', subtext: 'Nivel excepcional',
            color: 'text-emerald-400', bg: 'bg-emerald-500/10', border: 'border-emerald-500/20',
        }
        if (accuracy >= 75) return {
            text: 'Progresando adecuadamente', subtext: 'Buen ritmo de aprendizaje',
            color: 'text-cyan-400', bg: 'bg-cyan-500/10', border: 'border-cyan-500/20',
        }
        if (accuracy >= 50) return {
            text: 'Rendimiento estable', subtext: 'Sigue practicando para mejorar',
            color: 'text-amber-400', bg: 'bg-amber-500/10', border: 'border-amber-500/20',
        }
        return {
            text: 'Necesitas más práctica', subtext: 'No te rindas, la clave es la constancia',
